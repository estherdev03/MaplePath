...
def tef_to_nclc(score: LanguageScore) -> NCLCScore:
    return NCLCScore(
        speaking=_tef_speaking_to_nclc(score.speaking),
        writing=_tef_writing_to_nclc(score.writing),
        listening=_tef_listening_to_nclc(score.listening),
        reading=_tef_reading_to_nclc(score.reading),
    )


def _tef_speaking_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
def _tef_writing_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
def _tef_listening_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
def _tef_reading_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
# ================= TCF CANADA =====================================
def tcf_to_nclc(score: LanguageScore) -> NCLCScore:
    return NCLCScore(
        speaking=_tcf_speaking_to_nclc(score.speaking),
        writing=_tcf_writing_to_nclc(score.writing),
        listening=_tcf_listening_to_nclc(score.listening),
        reading=_tcf_reading_to_nclc(score.reading),
    )


def _tcf_speaking_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
def _tcf_writing_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
def _tcf_listening_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
def _tcf_reading_to_nclc(score: float | None) -> int:
    if score is None:
        return 0
...
def french_to_nclc(test: FrenchTest, scores: LanguageScore):
    match test:
        case FrenchTest.TCF:
            return tcf_to_nclc(scores)
        case FrenchTest.TEF:
            return tef_to_nclc(scores)
        case _:
            raise ValueError("{test} test score is not accepted.")


def nclc_to_points(
    scores: NCLCScore, is_married: bool = False, is_first_language: bool = False
):
