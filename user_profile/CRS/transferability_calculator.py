...
from user_profile.CRS.english_convertor import english_to_clb
from user_profile.CRS.french_convertor import french_to_nclc
...
def language_range(profile: UserProfile) -> Tuple[bool, bool, bool]:
    ...
    if (
        ...
        if not eng.clb_scores:
            ...
            eng.clb_scores = english_to_clb(test_name, scores)
    ...
    # french is first language
        ...
        if not french.nclc_scores:
            ...
            french.nclc_scores = french_to_nclc(test_name, scores)
...
def education_language_points_calc(profile: UserProfile) -> int:
    education = profile.education
    # invalid education
    if education and not education.from_canada and not education.eca_completed:
        edu_lang_points = 0
    else:
        _, all_clb7, all_clb9 = language_range(profile)
        if all_clb9:
...
def education_can_exp_points_calc(profile: UserProfile) -> int:
    # invalid experience
    ...
    if (
        ...
        or (not profile.education.from_canada and not profile.education.eca_completed)
...
def foreign_exp_lang_points_calc(profile: UserProfile) -> int:
    _, all_clb7, all_clb9 = language_range(profile)
...
def foreign_can_exp_points_calc(profile: UserProfile) -> int:
...
def trade_lang_points_calc(profile: UserProfile) -> int:
    if not profile.education or not profile.education.has_COQ:
        return 0
    else:
        all_clb5, all_clb7, _ = language_range(profile)
        if all_clb7:
            return 50
        elif all_clb5:
            return 25
